...
class WeeklyLactations():
    ...
    def create_individual_lactations(self):
        cols = self.lactation_1.columns     # 0 thru max of bd
        starts = range(1, 6)
        individual_lactations = {}

        for j in starts:    # iterates down the starts index ( lactations ) 1-6
            cow_dict = {}
            for i in cols:
                lactation_col_name = 'lactation_' + str(j)
                lactation_data = getattr(self, lactation_col_name)[i].copy()
                cow_dict[ str(i + 1)] = lactation_data

            individual_lactations[j] = cow_dict

        wy15_lact1 = individual_lactations[1][str(i + 1)]
        wy15_lact2 = individual_lactations[2][str(i + 1)]
