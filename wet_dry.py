...
rng.index+=1
#merge both start and stop with the blank array to get columns aligned 
...
#
start1=start2.T.astype(dtype='datetime64[ns]')
...
heady1= pd.to_datetime(heady,format='%m/%d/%Y')
datex=  pd.date_range(start=heady1,end=headx1,freq='d',name='datex')
#
headz1=(headx1-heady1)                                  #days between 9/1/2016 and 8/10/2018
headz2=headz1.days
head=headz2+1
#
...
for j in cols:
    for i in rows:
        ...
        maxstop=stop1.loc[:,j].max()
#  
        if((        pd.isnull(start) == False) 
            &   (   pd.isnull(stop ) == False)
            &   (   pd.isnull(cowDdate[j]) == True)     #cow not dead yet
            &   (   stop   == maxstop)                  #this stop IS the last one so far
            &   (   start   == maxstart)                #means both start and stop are in their last values
...
#
        if(     ( pd.isnull(startdry) == False)                 #start is not empty
            &   ( pd.isnull(stopdry)  == False)                 #stop  is not empty
...
bd['tf']=dead1                                              #adds the bool mask to the df
