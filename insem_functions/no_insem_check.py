...
Logic: If 'age_insem' is null and 'status' is 'M', select those rows.
...
class NoInsemCheck:
    ...
    def _create_get_no_insem_check(self):
        # Filter rows where 'age_insem' is null and 'status' is 'M'
        mask = (
            self.allx['age_insem'].isnull()
            & (self.allx['status'] == 'M')
        )
        # Include 'days_milking' field
        no_insem_check = self.allx.loc[mask, ['wy_id', 'i_date', 'age_insem', 'days_milking']].copy()
        ...
        # Sort by 'days_milking' ascending
        no_insem_check = no_insem_check.sort_values(by='days_milking', ascending=True).reset_index(drop=True)
